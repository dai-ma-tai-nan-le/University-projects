<template>
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','grade') || $check_field('add','grade') || $check_field('set','grade')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="年级" prop="grade">
					<el-input id="grade" v-model="form_sub['grade']" placeholder="请输入年级"
							  v-if="user_group === '管理员' || (form_sub['student_id'] && $check_field('set','grade')) || (!form_sub['student_id'] && $check_field('add','grade'))"></el-input>
					<div v-else-if="$check_field('get','grade')">{{form_sub['grade']}}</div>
				</el-form-item>
			</el-col>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_number') || $check_field('add','teacher_number') || $check_field('set','teacher_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师编号" prop="teacher_number">
						<div v-if="user_group !== '管理员'">
							{{ get_user_session_teacher_number(form_sub['teacher_number']) }}
							<!--<el-input id="business_name" v-model="form['teacher_number']" placeholder="请输入教师编号"-->
							<!--v-if="user_group === '管理员' || (form['student_id'] && $check_field('set','teacher_number')) || (!form['student_id'] && $check_field('add','teacher_number'))" :disabled="disabledObj['teacher_number_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','teacher_number')">{{form['teacher_number']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['student_id'] && $check_field('set','teacher_number')) || (!form['student_id'] && $check_field('add','teacher_number'))" id="teacher_number" v-model="form_sub['teacher_number']" :disabled="disabledObj['teacher_number_isDisabled']">
								<el-option v-for="o in list_user_teacher_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
							<el-select v-else-if="$check_field('get','teacher_number')" id="teacher_number" v-model="form_sub['teacher_number']" :disabled="true">
								<el-option v-for="o in list_user_teacher_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
						</div>
						<el-select v-else id="teacher_number" v-model="form_sub['teacher_number']">
							<el-option v-for="o in list_user_teacher_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
							</el-option>
						</el-select>
	...
	export default {
		...
		methods: {
			...
			async get_group_user_teacher_number() {
				this.form_sub["teacher_number"] = this.user.user_id;
