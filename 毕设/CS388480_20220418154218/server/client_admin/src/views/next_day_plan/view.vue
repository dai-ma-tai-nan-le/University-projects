		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','student_user') || $check_field('add','student_user') || $check_field('set','student_user')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学生用户" prop="student_user">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','student_user')) || (!form['next_day_plan_id'] && $check_field('add','student_user'))" :disabled="disabledObj['student_user_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','student_user')">{{form['student_user']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','student_user')) || (!form['next_day_plan_id'] && $check_field('add','student_user'))" id="student_user" v-model="form['student_user']" :disabled="disabledObj['student_user_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_number') || $check_field('add','student_number') || $check_field('set','student_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学号" prop="student_number">
					<el-input id="student_number" v-model="form['student_number']" placeholder="请输入学号"
							  v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','student_number')) || (!form['next_day_plan_id'] && $check_field('add','student_number'))" :disabled="disabledObj['student_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','full_name')) || (!form['next_day_plan_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','date') || $check_field('add','date') || $check_field('set','date')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="日期" prop="date">
					<el-date-picker :disabled="disabledObj['date_isDisabled']" v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','date')) || (!form['next_day_plan_id'] && $check_field('add','date'))" id="date"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','class_name') || $check_field('add','class_name') || $check_field('set','class_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="班级名称" prop="class_name">
					<el-input id="class_name" v-model="form['class_name']" placeholder="请输入班级名称"
							  v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','class_name')) || (!form['next_day_plan_id'] && $check_field('add','class_name'))" :disabled="disabledObj['class_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','completion') || $check_field('add','completion') || $check_field('set','completion')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="完成情况" prop="completion">
					<el-input id="completion" v-model="form['completion']" placeholder="请输入完成情况"
							  v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','completion')) || (!form['next_day_plan_id'] && $check_field('add','completion'))" :disabled="disabledObj['completion_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','self_scoring') || $check_field('add','self_scoring') || $check_field('set','self_scoring')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="自我打分" prop="self_scoring">
					<el-select id="self_scoring" v-model="form['self_scoring']"
						v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','self_scoring')) || (!form['next_day_plan_id'] && $check_field('add','self_scoring'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_number') || $check_field('add','teacher_number') || $check_field('set','teacher_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师编号" prop="teacher_number">
						<el-select v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','teacher_number')) || (!form['next_day_plan_id'] && $check_field('add','teacher_number'))" id="teacher_number" v-model="form['teacher_number']" :disabled="disabledObj['teacher_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','next_day_task') || $check_field('add','next_day_task') || $check_field('set','next_day_task')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="次日任务" prop="next_day_task">
					<el-input type="textarea" id="next_day_task" v-model="form['next_day_task']" placeholder="请输入次日任务"
						v-if="user_group === '管理员' || (form['next_day_plan_id'] && $check_field('set','next_day_task')) || (!form['next_day_plan_id'] && $check_field('add','next_day_task'))" :disabled="disabledObj['next_day_task_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','next_day_task')">{{form['next_day_task']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "next_day_plan_id",
				url_add: "~/api/next_day_plan/add?",
				url_set: "~/api/next_day_plan/set?",
				url_get_obj: "~/api/next_day_plan/get_obj?",
				url_upload: "~/api/next_day_plan/upload?",
				...
				disabledObj:{
					...
					"next_day_task_isDisabled": false,
				},
				// 用户列表
        ...
        if (this.form["date"].indexOf("-")===-1){
          this.form["date"] = this.$toTime(parseInt(this.form["date"]),"yyyy-MM-dd")
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/next_day_plan/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/next_day_plan/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/next_day_plan/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/next_day_plan/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/next_day_plan/view','get');
