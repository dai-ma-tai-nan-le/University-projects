    ...
    <div class="question">
        <el-card class="card">
            <div class="zxskp" v-for="index in list" :key="index">
                ...
                <div style="font-size: 20px;font-weight: 600;">共{{length}}个问题</div>
            </div>
        </el-card>
        <div class="wt">
            <div class="query">
                <el-form :inline="true" :model="form" class="demo-form-inline">
                    <el-form-item label="问题">
                        <el-input v-model="form.question" placeholder="请输入问题进行查询" @keyup.enter.native="query()" style="width: 500px;"/>
                    </el-form-item>
                    <el-form-item>
                    ...
                    <el-form-item>
                        <el-button type="primary" @click="fbwt()">发布问题</el-button>
                    </el-form-item>
                </el-form>
            </div>
                    ...
                    <el-card class="card1" shadow="hover">
                        <template #header>
							<div class="card-header">
								...
								<span style="margin-left:400px;display: block;margin-top: -20px;">问题</span>
								<div style="float: right;margin-top: 5px;">
									<el-button class="start shanchu" v-if="username===index.username" @click="zjdel(index)">删除</el-button>
                                    <el-button class="start shanchu" v-else-if="username===homeOwner" @click="fzdel(index)">删除</el-button>
								</div>
							</div>
						</template>
						<div class="minddle">
						...
						<div class="footer">
							...
							<div class="sangeann">
                                ...
                                <span style="margin-left: 400px;" @click="ckgd(index)">查看更多>></span>
							</div>
						</div>
                    </el-card>
                </div>
            ...
            <div v-if="length==0" style="height: 387px;text-align: center;padding-top: 100px;">
				...
				<div>还没有人提问，你先来提问吧！</div>
			</div>
            <div v-if="length==1" style="height: 263px;"></div>
            <div v-if="length==2" style="height: 58px;"></div>
        </div>
         <!--对话框-->
         <el-dialog title="发布问题" v-model="centerDialogVisible" width="50%" center>
            <el-input v-model="form1.textarea" :rows="15" type="textarea" placeholder="请输入问题" />
            <span style="margin-left: 285px;display: block;margin-top: 20px;">
                <el-button @click="centerDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="addwt()">发布</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
	...
	methods: {
        xianshi() {
            this.axios.post("/studyRoom/selectRoomId2", {
                username: username,
            })
            ...
            this.axios.post("/studyRoom/addwt", {
                username: username,
            ...
            this.axios.post("/studyRoom/selectWt", {
            ...
            let index1 = JSON.parse(JSON.stringify(index));
            router.push( {path: "/study_room/discuss1", query:{wtId:index1.wtId,homeOwner:this.homeOwner} }).then(() => {
				setTimeout(router.go(0), 0);
			});
        },zjdel(index){
            let index1 = JSON.parse(JSON.stringify(index));
            ElMessageBox.confirm(
                ...
                .then(() => {
                    this.axios.post('/studyRoom/zjdelwt', {
                        id: index1.wtId
                    })
                        .then(res => res.data).then(res => {
                            if (res.code === 200) {
