...
public class EmailUtils {
    public static void sendEmail(String username,String email,String code){
        //发送方
        ...
        MimeMessage message = createMsg(session,myAccount,username,email,code);
        //4.利用Transport 发送邮件
        try {
    ...
    private static MimeMessage createMsg(Session session, String myAccount,String username,String email,String code) {
        //使用session对象 获取待发送的邮件信息
        MimeMessage message = new MimeMessage(session);
        //3.设置发件人 收件人 标题 邮件内容 附件 发送时间等等
        try {
            //3.1发件人 from
            message.setFrom(new InternetAddress(myAccount, "学生自律管理系统", "utf-8"));
             ...
             * */
            message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(email,username, "utf-8"));
            //3.3生成邮件主题
            message.setSubject("忘记密码验证码","utf-8");
            String ip = Inet4Address.getLocalHost().getHostAddress();
//            String url = "http://"+ip+":8080/gp11xiaomi/activate?e="+ Base64Utils.encode(user.getEmail())+"&c="+Base64Utils.encode(user.getCode());
...
//            message.setContent(user.getUsername()+",你好<br>欢迎注册小米商城! 请点击链接进行激活:<a href='"+url+"'>"+url+"</a>","text/html;charset=utf-8");
            message.setContent(username+"\t您好！\n" +
                    "\n您正在进行密码修改，验证码有效期五分钟，请尽快完成修改！\n" +
                    "\n验证码为："+code+"\n" +
                     "。\n时间："+new Date()+"\n"+
                    "。\n备注：若非本人操作请忽略该邮件","text/html;charset=utf-8");
//            message.setContent("验证码为："+code,"text/html;charset=utf-8");
//            message.setContent("时间为："+new Date(),"text/html;charset=utf-8");
//            message.setContent(" <font color=“red”>备注：若非本人操作请忽略该邮件</font>","text/html;charset=utf-8");
            //设置邮件的发送时间 是立即发送
