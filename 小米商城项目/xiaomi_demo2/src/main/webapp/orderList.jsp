		...
		<div class="col-md-12">
			...
			<c:forEach items="${orderList}" var="order" varStatus="i">
				<tr>
					...
					<th>${order.otime}</th>
					<th>${order.oaddress}</th>
					<th>
						<button type="button" class="btn btn-danger btn-sm" onclick="showOrder('${order.uid}')">订单详情</button>
						<c:if test="${order.status eq 3 }">
							<button type="button" class="btn btn-warning btn-sm" onclick="changeStatus('${order.uid}')">确认收货</button>
						</c:if>
					</th>
				</tr>
			</c:forEach>
