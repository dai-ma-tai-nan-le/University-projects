...
<div class="panel panel-default" style="margin: 0 auto;width: 95%;">
	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<td>${od.order.oid}</td>
			<td>订单时间:</td>
			<td>${od.order.otime}</td>
		</tr>
		<tr>
			<td>收件人:</td>
			<td>${od.address.oname}</td>
			<td>联系电话:</td>
			<td>${od.address.ophone}</td>
		</tr>
		<tr>
			<td>送货地址:</td>
			<td>${od.address.odetail}</td>
			<td>总价:</td>
			<td>${od.order.ocount}</td>
		</tr>
		<tr>
				...
				<table align="center" cellpadding="0" cellspacing="0" width="100%"  class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${order}" var="item" varStatus="i">
						<tr align="center">
							<th>${i.count}</th>
							<th>
								<img src="${pageContext.request.contextPath}/goodsImgs/${item.goods.pimage}" width="50px" height="50px">
							</th>
							<th>${item.goods.pname}</th>
							<th>${item.goods.pstate}</th>
							<th>${item.goods.ptime}</th>
							<th>${item.goods.pprice}</th>
							<th>${item.onum}</th>
							<th>${item.ocount}</th>
						</tr>
					</c:forEach>
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				...
				<c:if test="${od.order.status eq 1 }">
					<button type="button" onclick="pay('${od.order.oid}','${od.order.ocount}')" class="btn btn-warning btn-sm">易付宝支付</button>
						&nbsp;&nbsp;
					<button type="button" onclick="payWeiXin('${od.order.oid}','${od.order.ocount}')" class="btn btn-success btn-sm">微信支付</button>
				</c:if>
			</td>
		</tr>
	</table>
